...
abstract contract MultiCall is IMultiCall {
    ...
    function executeCode(bytes memory _code, string memory _errorMsg) public override {
        (address _contract, uint256 _value, bytes memory _data) = abi.decode(_code, (address, uint256, bytes));
        (bool _success, ) = _contract.call{value: _value}(_data); //solhint-disable-line avoid-low-level-calls
